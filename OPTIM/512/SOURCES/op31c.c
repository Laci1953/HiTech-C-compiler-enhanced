...
void PrintSym(int id, sym_t* s)
{
  ...
  fprintf(stderr, "label %s ", /* s->label */ GetLabel(s));
...
/* !!! */
            ...
            if (valid &&
                pi->type == T_CALL &&
                strcmp(/* ps->label */ GetLabel(ps), "ncsv") == 0)
       	    {
                pi = pi->pNext;
                pop = pi->iLhs;
...
void pr_instruction(register inst_t *pi) {
    ...
    {
        if (/* (pi->iPSym->label[0]) */ *(p = GetLabel(pi->iPSym)))
            printf("%s:\n", /* pi->iPSym->label */ p);
        else
            printf("L%d:\n", pi->iSymId);
    }
        ...
        printf("%d:\n", pi->aux);     /* OPTIMISER[1]: shares printf call with above*/ /* m4: */
               ...
               key_f &&
               pi->type == T_CALL &&
               strcmp(/* ps_i->label */ GetLabel(ps_i), "csv") == 0)
        printf("push\tiy\npush\tix\nld\tix,0\nadd\tix,sp\n");
    else
    {
#ifdef STATISTICS
...
void sub_436e(register operand_t const *pi_arg) {
    ...
    switch (pi->type) {
        ...
        case T_CONST:
            if (pi->oPSym) {
		p = GetLabel(pi->oPSym);
                if (/* pi->oPSym->label[0] */ *p) 
                    printf("%s", /* pi->oPSym->label */ p);
                else 
