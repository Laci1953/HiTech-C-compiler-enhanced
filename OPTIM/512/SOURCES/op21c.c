...
register     inst_t 	 *ip;
    ...
    if (poR)
    {
        ReadMem((char*)&lo1, (char*)poR, 0, sizeof(operand_t));
        typeR = plo1->type;
        auxR = plo1->aux;
	validR = true;
    }
    ...
    if (poL)
    {
        ReadMem((char*)&lo2, (char*)poL, 0, sizeof(operand_t));
        typeL = plo2->type;
        auxL = plo2->aux;
	validL = true;
    }
...
kill:
        ...
        if (regValues[/* gPi->iRhs->aux */ auxR].type == (uint8_t)T_INVALID)
	{
	    ...
	    ReadMem((char*)&regValues[auxR], (char*)poL, 0, sizeof(operand_t));
#ifdef PRINT_REG_VAL
 PrintRegVal(auxR);
#endif
        }
	...
	if (seq2->iRhs && IsValid((char*)seq2->iRhs))
            ReadMem((char*)&lo3, (char*)seq2->iRhs, 0, sizeof(operand_t));
	else
	    plo3->type = 0x7F; /* invalid */

	if (seq2->iLhs && IsValid((char*)seq2->iLhs))
            ReadMem((char*)&lo4, (char*)seq2->iLhs, 0, sizeof(operand_t));
	else
	...
	if (op && IsValid((char*)op))
	{
	       ...
	       sub_1369(ip->iRhs) ) 
	    {
	        ...
	        PutByte((char*)op, OFF_aux, (char)auxL);
                ...
                ReadMem((char*)&regValues[auxL], (char*)ip->iRhs, 0, sizeof(operand_t));
#ifdef PRINT_REG_VAL
 PrintRegVal(auxL); 
#endif
                goto kill;
...
register     inst_t *pi1;
    ...
    if (/* gPi->iLhs->aux */ auxLL <= (uint8_t)REG_HL) {
        if ((l1 = sub_46b1(/* gPi->iRhs */ poRR, /* gPi->iLhs->aux */ auxLL)) != -1) {
            /* code hikes gPi->iLhs->aux before test !!! */
            /* regValues[gPi->iLhs->aux] = *gPi->iRhs; */
	    ReadMem((char*)&regValues[auxLL], (char*)poRR, 0, sizeof(operand_t));
#ifdef PRINT_REG_VAL
 PrintRegVal(auxLL); 
#endif
            /* gPi                       = gPi; ??? */
	    ...
	    PutByte((char*)(poRR), OFF_type, (uint8_t)T_REG);
            ...
            /* gPi->iRhs->aux            = l1; */
	    PutByte((char*)(poRR), OFF_aux, (uint8_t)l1);
	    auxRR = l1;
            ...
            if (/* gPi->iLhs->aux */ auxLL >= REG_BC) {
                ...
                /* pi1->iLhs->type = T_REG; */
   	        PutByte((char*)(pi1->iLhs), OFF_type, (uint8_t)T_REG);

                /* pi1->iRhs->type = T_REG; */
   	        PutByte((char*)(pi1->iRhs), OFF_type, (uint8_t)T_REG);

                /* pi1->iLhs->aux  = regHiLoMap[gPi->iLhs->aux].hiReg; */
   	        PutByte((char*)(pi1->iLhs), OFF_aux, regHiLoMap[/* gPi->iLhs->aux */ auxLL].hiReg);

                /* pi1->iRhs->aux  = regHiLoMap[gPi->iRhs->aux].hiReg; */
   	        PutByte((char*)(pi1->iRhs), OFF_aux, regHiLoMap[/* gPi->iRhs->aux */ auxRR].hiReg);
...
register     operand_t *po;
    ...
    po = gPi->iLhs;
    type = GetByte((char*)po, OFF_type);
    aux = GetByte((char*)po, OFF_aux);

    switch (gPi->aux) {
        default:
            if (sub_4682(/* gPi->iLhs */ po)) 
	    {
                gPi->aux = I_OR;
            } 
	    else if (/*(po = gPi->iLhs)->type */ type != (uint8_t)T_REG || 
			/* po->aux */ aux != (uint8_t)REG_A) 
	    {
                sub_4544(REG_AF);
                return false;
            } else 
        ...
        case I_OR:
            ...
            if (sub_4682(po)) {
                /* po->type = T_REG; */
		PutByte((char*)po, OFF_type, (uint8_t)T_REG);

                /* po->aux  = REG_A; */
		PutByte((char*)po, OFF_aux, (uint8_t)REG_A);
            }
        /* fall through */
        case I_SUB:
	caseP_SUB: /* common */
            if (/* gPi->iLhs->type */ type == (uint8_t)T_REG &&
		/* gPi->iLhs->aux */ aux == (uint8_t)REG_A) 
	    {
                if (gPi->aux == I_SUB) 
		{
                    break;
        ...
        case I_AND:
            ...
            if (/* po->type */ type == (uint8_t)T_CONST &&
		/* !po->oPOperand */ !GetWord((char*)po, OFF_oPOperand) && 
		/* (po->oVal & 0xff) == 255 ) { */
		( (uint8_t)GetByte((char*)po, OFF_oVal) == 255 ) /* -1 */) 
	    {
                gPi->aux = I_OR;
                /* po->type = T_REG; */
		PutByte((char*)po, OFF_type, (uint8_t)T_REG);
