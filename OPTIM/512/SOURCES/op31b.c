        ...
        for (;; HEAP(piF->pNext))
        {
            ...
            { /* m7: */
                ...
                case T_SYMBOL:
                    ...
                    if (tokType == T_EQU)
                    {
                        if (/* psF->label[0] */ *GetLabel(psF) != (char)'f')
                    ...
                    if (psect == DATA && /* psF->label[0] */ *GetLabel(psF) == (char)'S')
                    ...
                    if (psect == DATA || psect == BSS) {
                        ...
                        psF->p.pInst = /* NULL */ 0;
                        pr_psect(psect);
                        printf("%s:\n", /* psF->label */ GetLabel(psF));
                    }
                    else if (/* psF->label[0] */ *(p = GetLabel(psF)) == (char)'_')
                        name_fun = /* psF->label */ p; /* external name */
...
case_default:
                        ...
                        (l3 = piF->pAlt)->type == T_CONST && l3->pAlt->type == T_JP && l3->pAlt->aux == 0)
                    {
                        ...
                        while (l3 = l3->pAlt)
                        {
                            ...
                            {
                             /* *l3->iLhs = *piF->iLhs; */
                                ReadMem((char*)&tmpOp, (char*)piF->iLhs, 0, sizeof(operand_t));
                                WriteMem((char*)&tmpOp, (char*)l3->iLhs, 0, sizeof(operand_t));
                            }
