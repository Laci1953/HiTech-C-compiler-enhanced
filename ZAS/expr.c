...
rval_t exprRetVal;   /* 97a1 */
bool ignoreOverflow; /* 97a9 */

void unary(int16_t op, register rval_t *pc);                   /* 22 0EB3 +-- */
void binary(int16_t op, register rval_t *left, rval_t *right); /* 23 0F71 +-- */
...
rval_t *evalExpr() {
    ...
    for (;;) {
        ...
        if (G_FWD <= tokType && tokType <= G_INT) {
		    ...
		    GetString(tmpBuf, yylval.ySym->sName);
		    error("Undefined symbol %s", tmpBuf);
...
/**************************************************************************
 22 0eb3 +++
 **************************************************************************/
void unary(int16_t op, register rval_t *pv) {
    switch (op) {
    case T_MARKER:
        return;
    case T_UMINUS:
...
**************************************************************************/
void binary(int16_t op, register rval_t *left, rval_t *right) {
    switch (op) {
    case T_PLUS:
