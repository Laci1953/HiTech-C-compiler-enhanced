 ...
 *********************************************************/
bool sub_38CA(register node_t *p1a, int p2) {
    member_t *sb;
    member_t buf;
    member_t* p = &buf;
...
node_t *sub_39CA(register node_t *sa)
{
    ...
    if (sub_14F3(sa) == 3) {
        sprintf(tmpBuf, "%l", sa->info.np[0]->info.l);
        freeNode(sa->info.np[0]);
        sa->info.sv.s = (char*)MyAlloc(); /* allocMem(strlen(buf) + 1); */
						 /* create string */
        PutString(tmpBuf, sa->info.sv.s); /* strcpy(sa->info.sv.s, buf); */
 ...
 *********************************************************/
void sub_3A79(node_t *sa, register node_t *p2a) {
    int l1;
