 ...
 *
 *********************************************************/
void O_sub_5CF5(member_t *sb_arg, int p2) 
{
    member_t buf;
        ...
        if (sub_61AA(sb_arg, p2) == 0) 
	{
		...
		ReadMem(tmpBuf, sb->b_name, 32);
                prWarning("%s: large offset", /* sb->b_name */ tmpBuf);
	    }
        }
        break;
    case 5:
    ...
    case 4:
        /* sub_C57(sb_arg); */
	CallOverlay1(sub_C57, sb_arg, 32+5);
        break;
    }
...
node_t *sub_5DF6(register node_t *sa) 
{
    ...
    case ASSUB:
        if (sa->info.np[1]->a_c0 == CONST)
            ...
            sa->info.np[1] = /* sub_43EF(MINUS_U, sa->info.np[1], 0); */
			     (node_t*)CallOverlay3(sub_43EF, MINUS_U, sa->info.np[1], 0, 32+6);
        ...
        if (sa->info.np[0]->a_c0 == FCONST) 
	{
            freeNode(sa);
            sa = sa->info.np[0];
            l2 = MyAlloc(strlen(sa->info.sv.s) + 2); 
			/* l2 = allocMem(strlen(sa->info.sv.s) + 2); */
	    ReadMem(tmpBufAux, sa->info.sv.s, 32);
	    strcpy(tmpBuf, "-");
	    strcat(tmpBuf, tmpBufAux);
	    WriteMem(tmpBuf, l2, strlen(sa->info.sv.s) + 2);
            		/* strcat(strcpy(l2, "-"), sa->info.sv.s); */
	    free512(sa->info.sv.s);
            		/* free(sa->info.sv.s); */
            sa->info.sv.s = l2;
        }
    }
 ...
 *			      sub_3DC9
 *********************************************************/
node_t * O_sub_600E(register node_t *sa) 
{
    /* int l1; Not used */
	...
	sa = (node_t*)CallOverlay1(sub_3EAA,
				   (node_t*)CallOverlay1(sub_54B6, sa, 32+5),
				   32+5);
    } 
    while (byte_B013);
 ...
 *
 *********************************************************/
bool O_sub_60A8(register node_t *sa, node_t *p2a) 
{
    long l1;

    if (sa->a_c0 != 'C')
        /* return 0; */ ReturnOverlay2(0);

    if (nodesize(p2a) >= 4)
        ReturnOverlay2(sa->info.l >= 0 || 
		       /* sub_14F3(p2a) */ (char)CallOverlay1(sub_14F3, p2a, 32+5) == 1);

    l1 = (uint16_t)(1 << (nodesize(p2a) * 8));
    if (/* sub_14F3(p2a) */ (char)CallOverlay1(sub_14F3, p2a, 32+5) == 2) 
    {
        if (sa->info.l < 0)
            ReturnOverlay2(0);
    } 
 ...
 *********************************************************/
bool O_sub_6246(node_t *p1a, node_t *p2a) 
{
    ReturnOverlay2((bool)(nodesize(p1a) == nodesize(p2a) && 
		    /* sub_14F3(p1a) == sub_14F3(p2a) */
		    (char)CallOverlay1(sub_14F3, p1a, 32+5) == (char)CallOverlay1(sub_14F3, p2a, 32+5)));
}
...
{
    return /* sub_14F3(p1a) */ (char)CallOverlay1(sub_14F3, p1a, 32+5) == 1 && 
	   /* sub_14F3(p2a) */ (char)CallOverlay1(sub_14F3, p2a, 32+5) == 1;
}
 ...
 *********************************************************/
bool O_sub_62BE(register node_t *sa, node_t *p2a) 
{
    long l1, l2;
    char l3;
    ...
    ReturnOverlay2(l2 <= p2a->info.l && p2a->info.l < l1);
}
