 ...
 *    does hl = bc
 *********************************************************/
node_t * O_sub_54B6(register node_t *sa) {
    node_t *l1a;
    node_t *l2a;
    int l3;
    uint32_t l4;
    ...
    if (l3 != 0)
        sa->info.np[0] = /* sub_54B6(sa->info.np[0]) */ CallOverlay1(sub_54B6, sa->info.np[0], 32+5);

    if (l3 == 8)
        sa->info.np[1] = /* sub_54B6(sa->info.np[1]) */ CallOverlay1(sub_54B6, sa->info.np[1], 32+5);
	...
	(bool)CallOverlay2(sub_60A8, sa->info.np[1], sa->info.np[0]->info.np[0], 32+4)) 
    {
        l2a                   = sa->info.np[0];
        ...
        nodesize(l1a->info.np[0]) != 0 && nodesize(l1a->info.np[0]) < nodesize(l1a)) 
    {
        if (sa->info.np[1]->a_c0 == CONST &&
            (/* sub_14F3(l1a) */ (char)CallOverlay1(sub_14F3, l1a, 32+5) == 1 || 
	     /* sub_14F3(l1a->info.np[0]) */ (char)CallOverlay1(sub_14F3, l1a->info.np[0], 32+5) == 2)) 
	{
            if (/* sub_62BE(sa->info.np[0], sa->info.np[1]) */ 
		(bool)CallOverlay2(sub_62BE, sa->info.np[0], sa->info.np[1], 32+4)) 
	    {
                ...
                prWarning("mismatched comparision"); /* warningMsg = "mismatched comparision"; */ /* m8: */
        } 
	else if (sa->info.np[1]->a_c0 == CONV &&
                 /* sub_6246(sa->info.np[0]->info.np[0], sa->info.np[1]->info.np[0]) */
		 (bool)CallOverlay2(sub_6246, sa->info.np[0]->info.np[0], sa->info.np[1]->info.np[0], 32+4)) 
	{
            l1a            = sa->info.np[0];
    ...
    {
    case IDOP:
        /* return sub_4DA3(sa); */ ReturnOverlay1(sub_4DA3(sa));

    case DOT:
        /* return sub_4E8D(sa); */ ReturnOverlay1(sub_4E8D(sa));

    case NOT:
        /* return sub_4C8B(sa); */ ReturnOverlay1(sub_4C8B(sa));

    case LT:
    case GEQ:
        if (/* sub_14F3(sa->info.np[0]) */ (char)CallOverlay1(sub_14F3, sa->info.np[0], 32+5) == 2 && 
	    sub_4BE5(sa->info.np[1])) 
	{
            prWarning("degenerate unsigned comparision"); /* warningMsg = "degenerate unsigned comparision"; */
            sub_475C(sa);
        ...
        if (sa->info.np[1]->a_c0 == CONST && sa->info.np[1]->info.l == -1L) 
	{
            ...
            freeNode(sa);
            /* return sa->info.np[1]; */ ReturnOverlay1(sa->info.np[1]);
        }
        /* fall through */
    case ADD:
    case SUB:
    ...
    case RSHIFT:
        if (sa->info.np[1]->a_c0 == CONST && sa->info.np[1]->info.l == 0L)
            /* return sub_4FA8(sa); */ ReturnOverlay1(sub_4FA8(sa));

        if ((dopetab[sa->a_c0] & 0x80) && nodesize(sa->info.np[1]) != 1) 
	{
            l1a            = allocNode();
            l1a->pm        = /* sub_14F3(sa->info.np[1]) */ 
			     (char)CallOverlay1(sub_14F3, sa->info.np[1], 32+5) == 2 ? typeUChar : typeChar;
            l1a->a_c0      = TYPE;
            sa->info.np[1] = /* sub_43EF(CONV, sa->info.np[1], l1a); */
			     (node_t*)CallOverlay3(sub_43EF, CONV, sa->info.np[1], l1a, 32+6);
        }
        break;
    case MUL:
    case DIV:
    ...
    case ASMOD:
        if (sa->info.np[1]->a_c0 == CONST && 
	    /* sub_46F7(sa->info.np[1]->info.l) */ (char)CallOverlay1L(sub_46F7, sa->info.np[1]->info.l, 32+5) != 0)
            sub_4FCE(sa);
        /* fall through */
    case BAND:
    case ASAND:
        ...
        break;
    case CONV:
        /* return sub_508A(sa); */ ReturnOverlay1(sub_508A(sa));
    case PLUS_U:
        freeNode(sa);
        sa = sa->info.np[0];
        break;
    case GADDR:
        if (sa->info.np[0]->a_c0 == MUL_U)
            /* return sub_53EE(sa); */ ReturnOverlay1(sub_53EE(sa));
        break;
    case MUL_U:
