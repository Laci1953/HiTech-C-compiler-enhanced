 ...
 *********************************************************/
bool O_releaseNodeFreeList(void) 
{
    register node_t *sa;
 ...
 *********************************************************/
uint16_t O_sub_387A(register node_t *sa) 
{
    uint8_t loc;
    member_t buf;
    member_t* p = &buf;
...
void sub_3A79(register node_t *sa, node_t *p2a) 
{
    ...
    if (l1 != 0 && l1 < 4) 
    {
        ...
        if (/* sub_14F3(sa) */ (char)CallOverlay1(sub_14F3, sa, 32+5) == 1) 
	{
...
node_t *sub_3CDF(register node_t *sa) 
{
	...
	CallOverlay1(sub_2BD0, sa->info.np[0], 32+5);
        freeNode(sa);
        sa = sa->info.np[1];
    }
