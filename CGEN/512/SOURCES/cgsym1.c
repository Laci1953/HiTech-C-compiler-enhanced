...
void O_parseVariable(void) 
{
    ...
    PutWord((char*)sb, OFF_b_type, v = sub_627(getToken(), &tmp));
    PutWord((char*)sb, OFF_b_refl, tmp);
    /* l1a        = sub_600E(parseExpr()); */
    l1a = (node_t*)CallOverlay1(sub_600E, 
    ...
    if (/* sb->b_class */ GetByte((char*)sb, OFF_b_class) != TYPE) 
    {
	    ...
	    /* sb->b_flag */ b == 1)
            /* sub_5CF5(sb, l2); */
	    CallOverlay2(sub_5CF5, sb, l2, 32+4);
        else
...
void O_parseMembers(int p1) 
{
    ...
    for (;;) 
    {
        ...
        if (*l5 == ']')
            break;
        sb          = (member_t *)MyAlloc(sizeof(struct member)); /* Create member_t */
        /* sb->b_class = MEMBER; */
	...
	PutWord((char*)sb, OFF_b_type, v = sub_627(l5, &tmp));
	PutWord((char*)sb, OFF_b_refl, tmp);
        /* l4a        = sub_600E(parseExpr()); */
	l4a = (node_t*)CallOverlay1(sub_600E,
			   	    CallOverlay0(parseExpr, 32+5),
				    32+4);
...
void O_parseEnum() 
{
    ...
    {
        /* l1a = parseExpr(); */
	l1a = (node_t*)CallOverlay0(parseExpr, 32+5);
        ...
        if (l1a->a_c0 == DOT_DOT) 
	{
        ...
        /* l1a = sub_600E(l1a); */
	l1a = (node_t*)CallOverlay1(sub_600E, l1a, 32+4);
...
void sub_1EDF() 
{
    ...
    {
        for (sb = *l1b; 
	     sb != 0 && GetByte((char*)sb, OFF_b_ndpth) == nstdpth; 
	     sb = (member_t*)GetWord((char*)sb, OFF_b_next))
