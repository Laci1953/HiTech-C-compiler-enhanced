...
;
;<         strings>		<members       ><shadow code>
;                  ^		^               
;		   |            |		
;        	   TOP_STRINGS	BOTTOM_MBR	
;
DYNM_START	equ	1
DYNM_END	equ	0FF63H
...
MBR_SIZE	equ	22
;
TOP_STRINGS:	defw	DYNM_START	;top strings area (must be < BOTTOM_MBR)
BOTTOM_MBR:	defw	DYNM_END	;bottom operands area (must be > TOP_STRINGS)
;
...
_TotalSize:
	ld	hl,DYNM_END
	ld	bc,(BOTTOM_MBR)
	sbc	hl,bc			;HL=operands alloc size
	ex	de,hl			;DE=operands alloc size
	ld	hl,(TOP_STRINGS)	;HL=strings alloc size
	add	hl,de			;HL=total alloc size
	ret
;
;	Alloc member in Upper 64KB RAM
;
...
AllocDynMbr:
	...
	inc	a
	ld	hl,(BOTTOM_MBR)		;compare with bottom of allocated members area
	cp	h
	jr	z,full			
	or	a			;CARRY=0
	ld	bc,MBR_SIZE
	sbc	hl,bc
	ld	(BOTTOM_MBR),hl		;update bottom of allocated members area
...
;		or NULL
IF	TRACE
msgal:	defm	'alloc member returned=%04x'
	defb	0ah,0dh,0
ENDIF
_myallocmbr:
IF	TRACE
...
AllocDynStr:
	...
	ld	a,(BOTTOM_MBR+1)	;compare to bottom of allocated members area
	dec	a
	cp	h
	jr	z,fullstr
	push	hl			
