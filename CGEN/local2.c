...
void sub_5CF5(member_t *sb_arg, int p2) {
    ...
    case 1:
		...
		GetString(tmpBuf, sb->b_name);
                //prWarning("%s: large offset", tmpBuf );
		sprintf(tmpb, "%s: large offset", tmpBuf);
		prMsg(tmpb); 
	    }
        }
        break;
...
bool sub_61AA(member_t *sb_arg, int p2) {
    ...
    if ( /* rflag ignored !!! !rflag && */ isupper(p2) && sb->b_refl == 1 && sb->b_nelem <= 1) {
...
/*********************************************************
 * sub_628F OK++ PMO		Used in: sub_62BE
 *********************************************************/
