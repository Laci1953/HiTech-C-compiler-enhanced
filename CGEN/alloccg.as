...
ENDIF
	...
	psect data
;
TOP:	defw	DYNM_START	;top strings area (must be < DYNM_END)
;
	psect text
...
__sobuf::
	...
	defs	10	;TO move the rest of the code to 8000H !!!
;	
;	Alloc member, string in Upper 64KB RAM
;
...
_myalloc:
	ld	bc,20H			;BC=size
	ld	hl,(TOP)		;get top of allocated strings area
	ld	a,0FEH 			;compare to end of area (DYNM_END = FFxx)
	cp	h
	jr	z,isfull
	push	hl			
	add	hl,bc
	ld	(TOP),hl		;update top of allocated strings area
	pop	hl
...
;Virtual Pointer to allocated buffer (size = 20H)
;
;1aaaaaaaaaabbbbb : 
;
;where
;           bbbbb : virtual RAM bank
;0aaaaaaaaaa00000 : offset in RAM bank
...
BANK_SIZE	equ	8000H	;32KB
BANKS_CNT	equ	16	;16 banks of 32KB each are available
...
;void* myalloc(void)
;	
;	Alloc member or string (ALLWAYS 20H bytes)
;
;	returns HL=pointer (or NULL if no more available memory)
;
_myalloc:
	ld	a,(crtbnk)
IF	?ROMWBW
...
ENDIF
	jr	z,full
	SELECT
	ld	hl,(crtptr)	;fill record with 0
	xor	a
	ld	b,20H
loopz:	ld	(hl),a
	inc	hl
	...
	push	hl		;save-it
	ld	hl,(crtptr)	;increment pointer of available free area
	ld	bc,20H
	add	hl,bc
	ld	(crtptr),hl	;save-it
				;check-it against end-of-bank
	ld	bc,BANK_SIZE
	sbc	hl,bc
	jr	nz,retval
				;pointer reached end of bank, 
	ld	a,(crtbnk)	;increment current bank
	inc	a
	ld	(crtbnk),a
	ld	hl,0		;reset pointer of available free area
	ld	(crtptr),hl
retval:	pop	hl		;HL=virtual pointer
	ret
...
;
;	A = virtual bank (bbbbb = 0...31)
;	HL = offset in RAM bank (0aaaaaaa aaa00000)
;
;	returns HL = 1aaaaaaa aaabbbbb
;
ToVirtual::
	or	l
	ld	l,a		
	set	7,h		;HL = 1aaaaaaa aaabbbbb
	ret
;
;	Converts virtual pointer to bank & offset
;
;	HL = virtual pointer (1aaaaaaa aaabbbbb)
;
;	returns A = virtual bank (bbbbb = 0...31)
;		HL = offset (0aaaaaaa aaa00000)
;
FromVirtual:
	ld	a,l
	ld	b,a
