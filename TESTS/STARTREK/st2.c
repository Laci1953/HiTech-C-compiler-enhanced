...
void complete_maneuver(int, int);
void maneuver_energy(int);
...
void find_set_empty_place(char t, char *z1, char *z2);
...
extern int quad_y, quad_x;		/* Quadrant Position of Enterprise */
extern int shield;			/* Current shield value */
extern char stars;			/* Stars in quadrant */
extern int time_start;		/* Starting Stardate */
extern int time_up;		/* End of time */
extern int damage[9];			/* Damage Array */
extern int d4;			/* Used for computing damage repair time */
extern int ship_y, ship_x;		/* Current Sector Position of Enterprise, fixed point */
extern int stardate;		/* Current Stardate */
...
void dirdist_calc(void)
{
	...
	fprintf(stderr, "Please enter initial Y coordinate: ");
	a = TO_FIXED00(input_int());
	if (a < 0 || a > 900)
		return;

	fprintf(stderr, "Please enter final X coordinate: ");
	w1 = TO_FIXED00(input_int());
	if (w1 < 0 || w1 > 900)
		return;

	fprintf(stderr,"Please enter final Y coordinate: ");
