...
uint8_t getCh(void) {
    ...
    do {
        ...
        if (ungetCh) {
            ...
            if (s_opt)
                /*emitSrcInfo();*/ CallOverlay0(emitSrcInfo, 32+4);
            sInfoEmitted = false;
            lInfoEmitted = false;
 ...
 * 67: 35F7 PMO +++
 **************************************************/
uint8_t O_peekCh(void) {
    uint8_t ch;

    ungetCh = ch = skipWs();
    /* return ch; */ ReturnOverlay0(ch);
}

/**************************************************
 * 68: 3610 PMO +++
 **************************************************/
void O_skipStmt(uint8_t tok) {
...
void expect(uint8_t etok, char *msg) {
    ...
    if ((tok = /* yylex() */ (uint8_t)CallOverlay0(yylex, 32+4)) == etok)
        return;
    expectErr(msg);
